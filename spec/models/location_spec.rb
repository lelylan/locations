...
describe Location do
  ...
  context 'when connects a parent' do
    ...
    context 'with owned parent' do
      ...
      let(:resource) { FactoryGirl.create :location, parent: uri(parent), resource_owner_id: user.id.to_s }
  ...
  context 'when connects locations' do
    ...
    context 'with owned location' do
      ...
      let(:resource) { FactoryGirl.create :location, locations: [ uri(child) ], resource_owner_id: user.id.to_s }
    ...
    context 'with one owned location and one not owned location' do
      ...
      let!(:children)        { [ uri(owned_child), uri(not_owned_child) ] }
  ...
  context 'when updates connected locations' do
    ...
    context 'with owned locations' do
      ...
      before { resource.update_attributes!(locations: [ uri(child) ]) }
    ...
    context 'with not owned locations' do
      ...
      let!(:old_child) { resource.children.first }
      let!(:child)     { FactoryGirl.create :location }
      let(:update)     { resource.update_attributes!(locations: [ uri(child) ]) }
  ...
  context 'when connects a device' do
    ...
    context 'with owned device' do
      ...
      let!(:location)  { FactoryGirl.create :floor, :with_descendants, devices: [ uri(device) ], resource_owner_id: user.id.to_s }

      it 'connects the device' do
        location.devices.first.should == device.id
      end

      it 'stores the device BSON::ObjectID' do
        location.devices.first.class.should == Moped::BSON::ObjectId
      end
      ...
      context 'with children devices' do
        ...
        before { child_location.update_attributes!(devices: [ uri(child_device) ]) }
        ...
        context 'with descendant devices' do
          ...
          before { descendant_location.update_attributes(devices: [ uri(descendant_device) ]) }
    ...
    context 'with not owned device' do
      ...
      let(:resource) { FactoryGirl.create :floor, :with_descendants, devices: [ uri(device) ], resource_owner_id: user.id.to_s }
